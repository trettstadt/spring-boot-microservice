...
class BookingEntryRepositoryTest {
    ...
    void should_PerformCrudOperations() {
        // given (create)
        BookingEntryEntity givenEntity = BookingEntryEntity.builder().id(BigInteger.ONE).description("test").build();

        // when (create)
        BookingEntryEntity actualCreated = transactionTemplate.execute(_ -> bookingEntryRepository.save(givenEntity));
        ...
        // given / when (read)
        Optional<BookingEntryEntity> actualRead = transactionTemplate.execute(_ -> bookingEntryRepository.findById(BigInteger.ONE));
        ...
        // when (update)
        BookingEntryEntity actualUpdated = transactionTemplate.execute(_ -> bookingEntryRepository.save(actualRead.get()));
        ...
        // then
        List<BookingEntryEntity> actualAfterDelete = transactionTemplate.execute(_ -> bookingEntryRepository.findAll());
